...
//     // some untuk mencocokan
//     // filter menyaring
//     // maping memetakan
//     const match_loc = location.length === 0 || location.some((i) => i.id === item.location_id);
//     return match_loc;
// });
...
export default function Recommended() {
    ...
    const filterPackages = activeLoc ? packages.filter((item) =>item.location_id == activeLoc) : packages;
    ...
    <div className="container mx-auto px-20">
        ...
        <section className="py-2 -mt-12">
            <div className="flex justify-bentween items-center">
                <Gift className="w-10 h-10 text-yellow-400 fill-yellow-400 border rounded-full border-1 p-1 border-blue-700 bg-blue-700"/>
                <h2 className="px-2 text-2xl font-bold font-[roboto]"> New From Nirvana</h2>
            </div>
            ...
            <div className="flex gap-3 mb-6 px-8 font-[roboto]">
                {/* packages.some() akan ngecek apakah ada minimal satu package yang punya location_id sama dengan loc.id */}
                {location
                .filter((loc) => packages.some((p) => p.location_id === loc.id))
                .map((item) => (
                <button
                    key={item.id}
            ...
            <button onClick={() => scroll("left")}
                className="absolute left-45 md:left-60 top-515 -translate-y-1/2 z-10 bg-white/70 hover:bg-white border border-gray-300 
            ...
            >
                ...
                {/* <div className="flex-shrink-0">
                <Card title="Lagoon Spa" location="Canggu" price="IDR 200.000" />
                </div>
                <div className="flex-shrink-0">
            ...
            <button
                ...
                className="absolute right-35 md:right-65 top-515 -translate-y-1/2 z-10 bg-white/70 hover:bg-white  border border-gray-300 
                rounded-full shadow p-2">
